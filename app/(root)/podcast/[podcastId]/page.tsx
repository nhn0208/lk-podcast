...
const PodcastDetails = (
  { params }: { params: { podcastId: Id<'podcasts'>} }
) => {
  const { user } = useUser();
  ...
  const similarPodcasts = useQuery(api.podcasts.getPodcastByAuthorId, { authorId: podcast?.authorId || '' })
  ...
  return (
    <section className="flex w-full flex-col">
      ...
      <section className="mt-8 flex flex-col gap-5">
        ...
        {similarPodcasts && similarPodcasts.podcasts.length > 0 ? (
          <div className="podcast_grid">
            {similarPodcasts.podcasts?.map(({ _id, podcastTitle, podcastDescription, imageUrl }) => (
              <PodcastCard 
                key={_id}
